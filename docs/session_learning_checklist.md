...
Danh sách này giúp theo dõi tiến trình nắm bắt kiến thức của em trong phiên làm việc này. Chúng ta sẽ đi từng bước một.
...
> **Câu trả lời** Correlation_id chỉ là một đoạn mã được tạo ra, nên việc tìm thấy đoạn log chứa ID này thì cũng không giải quyết được vấn đề nào cả. Chúng ta sẽ phải load toàn bộ correlation id đó ra để kiểm xem thử họ lỗi ở đâu, lỗi như thế nào.
...
> **Câu trả lời:** Việc chuyển hóa log thành dashboards sẽ giúp ta hiểu nhanh hơn. 10k dòng log đó ta sẽ khó hiểu nó đang cho ra cái gì. Trong khi chuyển hóa thành các biểu đồ thì ta sẽ có cái nhìn trực quan hơn về các dòng dữ liệu. Ví dụ ta có 10 người chọn đáp án a,b,c ta sẽ phải đọc hết 10 dòng đó ta mới biết được bao nhiêu người chọn a hoặc b hoặc c. Trong khi chỉ cần 1 biểu đồ là ta đã có thể biết được chỉ trong vài giây.
...
> **Câu trả lời:** Dù dashboard có đẹp đến mấy thì việc theo dõi nó là rất mệt và tốn thời gian. Nếu ngồi nhìn 24/7 thì sẽ rất tốn sức. Nếu như có lỗi mà không ai hay thì sẽ gây ra thiệt hại. Alert Fatigue là 1 thuật ngữ để chỉ sự quá tải với cảnh báo. Khi có quá nhiều cảnh báo giả (ví dụ set up trong 1s hay 1phút thì rất dễ cảnh báo lỗi mà không phải lỗi thật) thì người dùng hoặc kĩ thuật sẽ bỏ qua nó hoặc tắt nó đi. Dẫn đến lỗi thật sẽ bị lờ đi.
### 2. Giải pháp (The Solution)
...
> **Ý nghĩa "for 5m":** Tại vì người ta cần một khoảng thời gian đủ dài để đánh giá được model đang hoạt động đúng chính xác hay không. Nếu set là 1s thì quá ngắn để nói model đó hoạt động lỗi vì đôi khi hệ thống sẽ phải có sai sót và nếu set quá dài thì mình sẽ không thể sửa kịp thời.
### 3. Ngữ cảnh rộng hơn (Broader Context)
